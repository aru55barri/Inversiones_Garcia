...
$id = $_SESSION['rol'];
$sql = mysqli_query($conn, "SELECT * FROM tbl_permisos where ID_OBJETO = 23 and ID_ROL = '$id'");
...
<style>
  ...
  font-size: 80px;
    ...
    <?php
    if (isset($_SESSION['registrartipo'])) {
        echo "<script>Notiflix.Notify.success('Registrado correctamente');</script>";
        unset($_SESSION['registrartipo']);
    }

    if (isset($_SESSION['EditarTipoProducto'])) {
        echo "<script>Notiflix.Notify.success('Editado correctamente');</script>";
        unset($_SESSION['EditarTipoProducto']);
    }
    if (isset($_SESSION['tipoProducto'])) {
      if ($_SESSION['tipoProducto'] == 'Si') {
        echo "<script>Notiflix.Notify.failure('Tipo Prodducto Eliminado Correctamente');</script>";
      } else {
        echo "<script>Notiflix.Notify.warning('No se pudo eliminar Tipo Producto porque esta asociado a un Producto');</script>";
      }
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered" id="table">
                    ...
                    <tbody>
                        ...
                        $resultado = mostrarTipoproducto();

                        foreach ($resultado as $tipoProducto) {
                            echo "<tr>";
                            echo "<td>" . $tipoProducto['id'] . "</td>";
                            echo "<td>" . $tipoProducto['descripcion'] . "</td>";
                            //CAMBIOS
                            if ($modificar == 1) {
                              echo "<th><a href='../src/modificar_tipo_producto.php?id=" . base64_encode($tipoProducto['id']) . "'> <i class='btn btn-round btn-info'><i class='fa-solid fa-pen-to-square'></i></a></th>";
                            }

                            //CAMBIOS
                            if ($eliminar == 1) { 
                           // echo "<th><button onclick=eliminar(" . $tipoProducto['ID_TIP_PRODUCTO'] . ")' class='btn btn-round btn-danger' type='submit'><i class='fas fa-trash-alt'></i></button></th>";
                            echo "<td><button class ='btn btn-round btn-danger' onclick='eliminar(" . $tipoProducto['id'] . ")'><i class='fas fa-trash-alt'></i></button></td>";
                            }
                            echo "</tr>";
                        }
                        ...
                        VALUES(null,'$fecha','$IDUS',23, 'INGRESO','$Usuarioo INGRESÓ A TABLA TIPO DE PRODUCTO')";
...
<script>
    function eliminar(id) {
        ...
        swal.fire({
            ...
            if (result.value) {
                window.location.href = '../Login/eliminarAuxiliar.php?id=' + id + '&tabla=tipoProducto';
            }
        })
