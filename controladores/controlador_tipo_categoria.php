<?php
    require_once '../modelos/modelo_tipo_categoria.php';
    ...
    function InsertarCliente($nombre){
        ...
        $modeloCliente = new Cliente();
        $resultado = $modeloCliente->InsertarCliente($nombre);
        return $resultado;
    }
    ...
    {
        $modeloCliente = new Cliente();
        $resultado = $modeloCliente->EditarCliente($id,$nombre);
        return $resultado;
    }
...
class ClientesContralador
{
    ...
    static public function mostrarTipocategoria()
    {

        $usuarios = new ModeloPrincipal();
        $matrizUsuario = $usuarios->mostrargeneral("select * from tipo_categoria");
        global $modificar;
        global $eliminar;
         $ii = 0;
        foreach ($matrizUsuario as $key => $value) {
            foreach ($value as $registro) {
                ...
                if ($modificar == 1){
                echo "<th><a href=../src/modificar_tipo_categoria.php?id=" . $registro['id'] . " class='btn btn-round btn-info'><i class='fas fa-pen-square' style='color: white'></i></a></th>";
                }
                if ($eliminar == 1) {
                  echo "<th><button class ='btn btn-round btn-danger' onclick='eliminar(".$registro['id'].")'> <i class='fas fa-trash-alt'></i></a></th>";
                  }
                  echo "</tr>";                      
            ...
            VALUES(null,'$fecha','$IDUS',2 ,'INGRESO', 'EL USUARIO INGRESA A TABLA TIPO CATEGORIA')";
